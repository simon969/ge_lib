...
from ge_lib.plaxis.plxresults.Plaxis3dResults2018 import Plaxis3dResults2018
...
# file:///C:/Program%20Files/Plaxis/PLAXIS%203D/manuals/english/output_objects/objects_EmbeddedBeam.html
    ...
    def getInterfaceResults(self,
        ...
        for phase in self.phaseOrder:
            ...
            try:
                ...
                Mat = self.g_o.getresults(phase, self.g_o.ResultTypes.Interface.MaterialID, 'node')
                ...
                w.rowsOut = [formats.format(pname, pident, x, y, z, mat, ux, uy, uz, ut, pux, puy, puz, put, u1, u2, u3, ens, tns, ss, rss, pe, pa, pst, pw, su, esu)
                    ...
                    in zip(phaseName, phaseIdent, X, Y, Z, Mat, Ux, Uy, Uz, Ut, PUx, PUy, PUz, PUt, U1, U2, U3, InterfaceEffNormalStress, InterfaceTotNormalStress, InterfaceShearStress, InterfaceRelShearStress, PExcess, PActive, PSteady, PWater, Suction, EffSuction)]
                w.writeOutput
    ...
    def getNodeToNodeAnchorResults(self,
        ...
        for phase in self.phaseOrder:
            ...
            try:
                X = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.X, 'node')
                Y = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.Y, 'node')
                Z = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.Z, 'node')
                
                Ux = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.Ux, 'node')
                Uy = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.Uy, 'node')
                Uz = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.Uz, 'node')
                Ut = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.Utot, 'node')

                PUx = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.PUx, 'node')
                PUy = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.PUy, 'node')
                PUz = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.PUz, 'node')
                PUt = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.PUtot, 'node')

                U1 = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.U1, 'node')
                U2 = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.U2, 'node')
                U3 = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.U3, 'node')
                
                Force3D = self.g_o.getresults(phase, self.g_o.NodeToNodeAnchor.AnchorForce3D, 'node')
                ...
                w.rowsOut = [formats.format(pname, pident, x, y, z, ux, uy, uz, ut, pux, puy, puz, put, u1, u2, u3, f3d)
                                 for pname, pident, x, y, z, ux, uy, uz, ut, pux, puy, puz, put, u1, u2, u3, f3d in zip(phaseName, phaseIdent, X, Y, Z, Ux, Uy, Uz, Ut, PUx, PUy, PUz, PUt, U1, U2, U3, Force3D)]
                w.writeOutput
    ...
    def getFixedEndAnchorResults(self,
        ...
        for phase in self.phaseOrder:
            ...
            try:
                X = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.X, 'node')
                Y = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.Y, 'node')
                Z = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.Z, 'node')
                
                Ux = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.Ux, 'node')
                Uy = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.Uy, 'node')
                Uz = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.Uz, 'node')
                Ut = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.Utot, 'node')

                PUx = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.PUx, 'node')
                PUy = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.PUy, 'node')
                PUz = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.PUz, 'node')
                PUt = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.PUtot, 'node')

                U1 = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.U1, 'node')
                U2 = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.U2, 'node')
                U3 = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.U3, 'node')
                
                Force3D = self.g_o.getresults(phase, self.g_o.FixedEndAnchor.AnchorForce3D, 'node')
