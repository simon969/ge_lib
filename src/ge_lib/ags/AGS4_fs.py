...
def AGS4_to_dict(filepath_or_buffer, encoding='utf-8', get_line_numbers=False, rename_duplicate_headers=True):
    ...
    try:
        ...
        for i, line in enumerate(f, start=1):
            ...
            # .decode() needed if its a binary file that hasn't been decoded
            try:
                temp = line.decode(encoding=encoding, errors="replace").rstrip().split('","')
...
def AGS4_to_dataframe(filepath_or_buffer, encoding='utf-8', get_line_numbers=False, rename_duplicate_headers=True):
    ...
    Parameters
    ----------
    input_filepath_or_buffer : str, StringIO
...
def AGS4_to_excel(input_filepath_or_buffer, output_filepath_or_buffer, encoding='utf-8', rename_duplicate_headers=True, sort_tables=False):
    ...
    # Extract AGS4 file into a dictionary of dictionaries
    tables, headings = AGS4_to_dataframe(input_filepath_or_buffer, encoding=encoding,
                                         rename_duplicate_headers=rename_duplicate_headers)
    ...
    # Write to Excel file
    print (type(output_filepath_or_buffer))

    if _is_excel_writer_like(output_filepath_or_buffer):
        writer = output_filepath_or_buffer
        close_excel = False
    else:
        writer = ExcelWriter(output_filepath_or_buffer, engine='openpyxl')
        close_excel = True
...
def check_file(filepath_or_buffer, dictpath_or_buffer, encoding='utf-8', rename_duplicate_headers=True):
    ...
    if _is_file_like (dictpath_or_buffer):
        d = dictpath_or_buffer
        close_d = False
    else:
        # Read file with errors="replace" to catch UnicodeDecodeErrors
        d = open(dictpath_or_buffer, "r", encoding=encoding, errors="replace")
        close_d = True
    ...
    # Line checks
        ...
        for i, line in enumerate(f, start=1):
            ...
            if ('AGS Format Rule 3' in ags_errors) and ('AGS3' in ags_errors['AGS Format Rule 3'][0]['desc']):
                ags_errors = add_meta_data(f, d, ags_errors=ags_errors)
                return ags_errors
    ...
    tables_std_dict, _ = AGS4_to_dataframe(d)
...
def _is_excel_writer_like(obj):
    
    from pandas import ExcelWriter
    print (obj.__class__.__name__)

    if hasattr(obj,"engine"):
        if obj.engine=="openpyxl":
            return True
    if type(obj) == type(ExcelWriter): 
        return True
    if obj.__class__.__name__ == "<class 'pandas.io.excel._openpyxl.OpenpyxlWrite'>":
        return True
    
    return False

def add_meta_data(input_file, dict_file, ags_errors={}):
    '''Add meta data from input file to error list.

    Parameters
    ----------
    input_file : 
        AGS4 file or any object with a read() method (such as an open file or StringIO).
    dict_file : str
    ...
    if input_file is not None:
        if hasattr(input_file,'name'): 
            check.add_error_msg(ags_errors, 'Metadata', 'File Name', '', f'{os.path.basename(input_file.name)}') 
            if hasattr(input_file,'size'):
                check.add_error_msg(ags_errors, 'Metadata', 'File Size', '', f'{int(input_file.size/1024)} kB')
        else:
            check.add_error_msg(ags_errors, 'Metadata', 'File Name', '', f'{os.path.basename(input_file)}')
            check.add_error_msg(ags_errors, 'Metadata', 'File Size', '', f'{int(os.path.getsize(input_file)/1024)} kB')
        check.add_error_msg(ags_errors, 'Metadata', 'Checker', '', f'python_ags4 v{__version__}')

    if dict_file is not None:
        if hasattr(dict_file,'name'):
            check.add_error_msg(ags_errors, 'Metadata', 'Dictionary', '', f'{os.path.basename(dict_file.name)}')
        else:
            check.add_error_msg(ags_errors, 'Metadata', 'Dictionary', '', f'{os.path.basename(dict_file)}')
    check.add_error_msg(ags_errors, 'Metadata', 'Time (UTC)', '', f'{datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")}')

    return ags_errors

def pick_standard_dictionary(tables=None, dict_version=None)->str:
    ...
    dict_name =  get_dict_name(tables, dict_version)
...
def get_dict_name(tables=None, dict_version=None)->str:
    ...
    try:
        if dict_version not in AGS_DICS:
            TRAN = tables['TRAN']
            dict_version = TRAN.loc[TRAN.HEADING.eq('DATA'), 'TRAN_AGS'].values[0]
        
        if dict_version in AGS_DICS:
            dict_file = AGS_DICS[dict_version]
        else:
            ...
            dict_file = 'Standard_dictionary_v4_1_1.ags'
